...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        lastBodyPos = transform.position;
        if (nbLegs > 3 && bodyOrientation && GetComponentInParent<StarterAssets.ThirdPersonController>().Grounded)
        {
            Vector3 v1 = legTargets[0].position - legTargets[1].position;
            Vector3 v2 = legTargets[2].position - legTargets[3].position;
            Vector3 normal = Vector3.Cross(v1, v2).normalized;
            Vector3 up = Vector3.Lerp(lastBodyUp, normal, 1f / (float)(smoothness + 10));
            transform.up = up;
